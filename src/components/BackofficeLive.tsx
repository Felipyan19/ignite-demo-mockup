      ...
      <section className="backoffice-panel demo-backoffice-theme overflow-hidden rounded-[24px] bg-[var(--bo-bg)] shadow-[0_22px_60px_rgba(0,0,0,.28)]">
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="mt-3.5">
            <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-[var(--bo-muted)]">Reservas de hoy</p>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-[var(--bo-muted)]">
                      {row.service} · {row.schedule} · {row.time}
                    </p>
                  </div>
                  {row.isNew && (
                    <span className="shrink-0 rounded-full bg-violet-600 px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide text-white">Nueva</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="mt-3.5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <IntegrationPill logo="calendar" label="Google Calendar" />
              <IntegrationPill logo="bold" label="Bold" />
...
function IntegrationPill({ logo, label }: { logo: 'whatsapp' | 'calendar' | 'bold' | 'crm'; label: string }) {
  return (
